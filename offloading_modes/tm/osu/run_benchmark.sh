...
for COLL in "${COLLECTIVES[@]}"; do
  ...
  mpirun --prefix "$PR" $TMARG -x PATH -x LD_LIBRARY_PATH -hostfile "$HOSTFILE" --map-by node --bind-to core -np "$NPROCS" --mca pml ucx \
    --mca osc ucx --mca coll_hcoll_np 1  --mca coll_"$LIB"_enable 1 --mca coll_"$LIB"_priority 100 \
    "$BIN" > "$OUTPUT_FILE"
done
