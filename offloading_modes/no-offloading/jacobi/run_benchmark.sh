...
mpirun --prefix "$PR" -x PATH -x LD_LIBRARY_PATH -hostfile "$HOSTFILE" $THRESH --map-by node --bind-to core -np "$NPROCS" --mca pml ucx --mca osc ucx \
  --mca coll_"$LIB"_enable 1 --mca coll_"$LIB"_priority 100 "$BIN" 32768 100  > "$OUTPUT_FILE"
